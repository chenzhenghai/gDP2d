...
struct mesh {
	...
	int flipcount;           /*QM-Number of flips performed*/
...
int triunsuitable(vertex triorg, vertex tridest, vertex triapex, REAL size)
			...
			for (j = STARTINDEX; argv[i][j] != '\0'; j++) {
				...
				if (argv[i][j] == 'u') {
					...
					if (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) ||
						...
						workstring[k] = '\0';
						b->maxlength = (REAL)strtod(workstring, (char **)NULL);
						if (b->maxlength <= 0.0) {
							printf("Error:  Maximum edge length must be greater than zero.\n");
							triexit(1);
						}
					}
				}
...

{
	...
	if (b->vararea || b->fixedarea || b->usertest) {
		...
		if (b->usertest) {
			...
			if (triunsuitable(torg, tdest, tapex, b->maxlength)) {
				enqueuebadtri(m, b, testtri, minedge, tapex, torg, tdest);
				return;
			}
		}
	}
	// printf("smallest angle = %.4f %.4f\n", angle, b->goodangle);
	/* Check whether the angle is smaller than permitted. */
	if (angle > b->goodangle) {
		//	  printf("%f %f %f %f %f %f\n", tapex[0], tapex[1], torg[0], torg[1], tdest[0], tdest[1]);
		...
		if ((vertextype(base1) == SEGMENTVERTEX) &&
			...
			if (testsub.ss == m->dummysub) {
				...
				//printf("\n");
				//printf("%f %f %f %f ", org1[0], org1[1], dest1[0], dest1[1]);
				//printf("%f %f %f %f\n", org2[0], org2[1], dest2[0], dest2[1]);
...

{
	...
	samplesperblock = ((__int64)m->samples * TRIPERBLOCK - 1) / m->triangles.maxitems + 1;
	/* We'll draw ceiling(samples * itemsfirstblock / maxitems) random samples */
	/*   from the first block of triangles.                                    */
	samplesleft = ((__int64)m->samples * m->triangles.itemsfirstblock - 1) /
				...
				if (doflip) {
					...
					if (b->vararea) {
						if ((areabound(top) <= 0.0) || (areabound(horiz) <= 0.0)) {
							...
							/*   long, __int64 way from their original location due to flips. */
	...
	while ((m->badsubsegs.items > 0) && (m->steinerleft != 0)) {
		...
		while ((encloop != (struct badsubseg *) NULL) && (m->steinerleft != 0)) {
			...
			if (!deadsubseg(currentenc.ss) &&
				...
				sorg(currentenc, eorg);
				sdest(currentenc, edest);
				REAL tt = counterclockwise(m, b, eorg, edest, newvertex);
				//if(tt != 0)
				//	printf("no use\n");
...

{
	...
	m->firstnonemptyq = -1; //////////77
	badTriangle = 0;
	tallyfaces(m, b);//////////////77
	if (!b->quiet)
		printf("badTriangle = %i\n", badTriangle);
...
#ifdef ANSI_DECLARATORS
void writeneighbors(struct mesh *m, struct behavior *b, int **neighborlist, int ** segment2trilist)
#else /* not ANSI_DECLARATORS */
...

{
	...
	*(int *)(m->dummytri + 6) = -1;
	...
	struct osub subsegloop, subsegsym;
	struct otri neighbortri;
	__int64 subsegnumber;
	//if (*segment2trilist == (int *)NULL) {
		*segment2trilist = (int *)trimalloc((int)(m->subsegs.items *
			sizeof(int)));
	//}

	int *snlist = *segment2trilist;
	index = 0;
	traversalinit(&m->subsegs);
	subsegloop.ss = subsegtraverse(m);
	subsegloop.ssorient = 0;
	...
	while (subsegloop.ss != (subseg *)NULL) {
		...
		neighbor1 = *(int *)(neighbortri.tri + 6);
		if (neighbor1 != -1)
		{
			snlist[index++] = neighbor1;
		}
		else
		{
			ssymself(subsegloop);
			stpivot(subsegloop, neighbortri);
			neighbor2 = *(int *)(neighbortri.tri + 6);
			snlist[index++] = neighbor2;
		}
		subsegloop.ss = subsegtraverse(m);
		subsegnumber++;
	}
...

{
	...
	if (b.neighbors) {
#ifdef TRILIBRARY
		writeneighbors(&m, &b, &out->neighborlist, &out->segment2trilist);
#else /* not TRILIBRARY */
