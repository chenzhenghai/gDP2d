...
void GenerateRandomInput(char* input_path, int numOfPoints, int numOfSegments,int seed, int distribution,triangulateio *result, double min_input_angle, bool checkinter)
{    
	...
	while(!otag && shift < numOfSegments)
	{
		...
		if(min_input_angle >= 60.0)
			strs << input_path << "d" << distribution << "_s" << seed << "_p" << numOfPoints << "_c" << (numOfSegments - shift);
		else
			strs << input_path << "d" << distribution << "_s" << seed << "_p" << numOfPoints << "_c" << (numOfSegments - shift) << "_with_minimum_input_angle_" << min_input_angle;
